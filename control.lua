...
function createForces()
    ...
    for i=1,factionCount do
        ...
        if not game.forces["biter_faction_"..i] then
            ...
            storage.factionList[i] = game.create_force("biter_faction_"..i)
            storage.nests[i] = {}
            local forcecreated = storage.factionList[i]
            if debugMode then game.print("created biter faction: "..forcecreated.name) end
            forcecreated.ai_controllable = true
            forcecreated.set_evolution_factor(game.forces["enemy"].get_evolution_factor())
            forcecreated.set_evolution_factor_by_pollution(game.forces["enemy"].get_evolution_factor_by_pollution())
            forcecreated.set_evolution_factor_by_time(game.forces["enemy"].get_evolution_factor_by_time())
            forcecreated.set_evolution_factor_by_killing_spawners(game.forces["enemy"].get_evolution_factor_by_killing_spawners())
            ...
            storage.factionList[i] = game.forces["biter_faction_"..i]
            storage.nests[i] = {}
            local forcecreated = storage.factionList[i]
            if debugMode then game.print("created biter faction: "..forcecreated.name) end
            forcecreated.ai_controllable = true
...
function getChunkNestsAndAllocate(chunkarea, surface)
    if not storage.factionList then game.print("error in getChunkNestsAndAllocate 1") return end
    if #storage.factionList < 1 then game.print("error in getChunkNestsAndAllocate 2") return end

    local factionIndex = math.random(#storage.factionList)

    if not factionIndex or not storage.factionList[factionIndex] then game.print("error in getChunkNestsAndAllocate 3 ") return end
    ...
    for i, entity in pairs(entitiesList) do
        ...
        if entity and entity.valid then
            ...
            if not destroyed and entity.type == "unit-spawner" then
                if not storage.nests[i] then storage.nests[i] = {} end
                storage.nests[i][entity.unit_number] = entity --store the entity by its unit number in the force's list.
                if debugMode then game.print("Faction "..factionIndex.." has ".. table_size(storage.nests[i]).." nests!") end
            end
...
function runTickHandler(event)
            ...
            if (storage.chunkProgress < storage.chunkIteratorLimit) then
                ...
                if debugMode then
                    if(storage.chunkProgress % 100 == 0) then
                        game.print("Biter Faction chunk processing progress... "..storage.chunkProgress.." out of "..storage.chunkIteratorLimit)
                    end
                end
    ...
    for i, force in pairs( storage.factionList) do
        ...
        --compare tick
        if not storage.nextAttackTick[i] then storage.nextAttackTick[i] = event.tick end
        ...
        --ensure there is a list of nests. if there isn't quickly generate one to get started..
                ...
                if not storage.scheduledAttack then storage.scheduledAttack = {} end
                storage.scheduledAttack[i] = 1 --set this faction's index to start an attack.

                if not storage.attackProgress then storage.attackProgress = {} end
        ...
        if (storage.scheduledAttack and storage.scheduledAttack[i] == 1) then
            ...
            if storage.attackProgress[i] >= maxCount then  --if we've ticked up enough nests then we are done.
                if(debugMode) then game.print("Biter faction "..i.." finished attacking with "..storage.attackProgress[i].." nests!") end
                if(debugMode) then game.print("Biter faction has "..table_size(storage.nests[i]).." nests") end
